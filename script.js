...
btnTrack.addEventListener("click", () => {
  ...
  if (isTracking) {
    ...
    btnTrack.innerHTML =
      '<span class="material-icons-round icon">stop_circle</span> Stop Tracking';
...
function updateUI(data) {
  ...
  batteryText.innerHTML = `${batt}<span class="unit">%</span>`;
  ...
  if (gpsIcon) {
    if (data.gps === "Locked") {
      ...
      gpsIcon.textContent = "near_me_disabled"; // Slashed icon
  ...
  const serverReportedFall =
    data.is_fallen === true || data.is_fallen === "true" || data.is_fallen == 1;
